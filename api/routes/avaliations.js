...
router.param("productId", (_, res, next, value) => {
  ...
  if (!id)
    return res.status(400).json({ error: `invalid productId: "${value}"` });
  else next();
});
...
router.param("userId", (_, res, next, value) => {
  const id = parseInt(value);
  if (!id)
    return res.status(400).json({ error: `invalid user id: "${value}"` });
  else next();
});
...
router.get(
  `${endpoint}/produtos/:productId/usuarios/:userId`,
  auth.check_token,
  function (req, res) {
    ...
    avaliation
      ...
      .catch((err) => {
        return res.status(500).json({
          message:
            "Erro ao recuperar avaliações de usuário em um produto - " +
            err.message,
        });
      });
  }
...
router.post(endpoint, auth.check_token, auth.check_admin, (req, res) => {
  ...
  avaliation.add(req.body).then((data) => {
    if (data) return res.status(201).json(data);
      ...
      return res
        .status(500)
        .json({ error: `Erro ao inserir avaliação ${req.body}` });
  });
});

// DEL: Delete an avaliation
router.delete(`${endpoint}/:avaliationId`, auth.check_token, (req, res) => {
...
router.put(`${endpoint}/:avaliationId`, auth.check_token, (req, res) => {
  const id = parseInt(req.params.avaliationId);
  avaliation
    .update(id, req.body)
    .then((count) => {
